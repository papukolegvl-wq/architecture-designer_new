...
export async function generateArchitectureCase(
  ...
  const domains = [
    'FinTech (высокочастотный трейдинг)',
    'E-commerce (глобальный маркетплейс)',
    'SpaceTech (управление группировкой спутников)',
    'HealthTech (анализ медицинских данных реального времени)',
    'GameDev (MMORPG с миллионами игроков)',
    'IoT (умный город и миллионы сенсоров)',
    'AdTech (RTB-биддеры с p99 < 50ms)',
    'Logistics (автономные склады и доставка дронами)',
    'EdTech (интерактивная платформа обучения)',
    'Media (стриминг видео высокого разрешения)',
  ...
  const twists = [
    'Миграция с 20-летнего монолита',
    'Ограниченная пропускная способность сети (Satellite-only)',
    'Строгие законы о суверенитете данных (Data Residency)',
    'Условия экстремально высокого трафика (Flash Sales)',
    'Нулевое доверие (Zero-Trust) по умолчанию',
    'Необходимость работы в оффлайн-режиме с последующей синхронизацией',
...
- Уровень сложности: ${difficulty.toUpperCase()}

Кейс должен быть структурированным и содержать:
1. **Бизнес-цели (Business Goals)**: Глобальные KPI.
...
Верни ответ ТОЛЬКО в формате JSON:
  {
    "id": "${Date.now()}",
    "title": "Хлесткое и интересное название",
    "difficulty": "${difficulty}",
    "description": "Богатое деталями описание бизнес-ситуации и предыстории",
