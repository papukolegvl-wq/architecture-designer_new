...
import { ComponentData, EmailServiceVendor } from '../types'

interface EmailServiceConfigPanelProps {
    node: Node
    onUpdate: (nodeId: string, config: { vendor: EmailServiceVendor }) => void
    onClose: () => void
}

const vendors: Array<{ value: EmailServiceVendor; label: string; description: string }> = [
    { value: 'sendgrid', label: 'SendGrid', description: 'Популярный сервис доставки email' },
    { value: 'aws-ses', label: 'AWS SES', description: 'Amazon Simple Email Service' },
    { value: 'mailgun', label: 'Mailgun', description: 'Email сервис для разработчиков' },
    { value: 'postmark', label: 'Postmark', description: 'Доставка транзакционных писем' },
    { value: 'mailchimp', label: 'Mailchimp', description: 'Маркетинговые рассылки и транзакционные письма' },
]

export default function EmailServiceConfigPanel({
    node,
    onUpdate,
    onClose,
}: EmailServiceConfigPanelProps) {
    const data = node.data as ComponentData
    const [vendor, setVendor] = useState<EmailServiceVendor | undefined>(
        data.emailServiceConfig?.vendor
    )
