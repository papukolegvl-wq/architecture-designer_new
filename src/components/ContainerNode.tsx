...
import { Package, Link as LinkIcon, Link2, Sparkles, CheckCircle, AlertTriangle, Settings } from 'lucide-react'
...
function ContainerNode({
  ...
  const updateContainerSize = React.useCallback(() => {
    ...
    let insideNodes = allNodes.filter(node => {
      if (node.id === id || node.type === 'system') return false
  ...
  const borderColor = selected ? '#4dabf799' : containerColor
  ...
  return (
    <div
      style={{
        width: '100%',
        height: '100%',
        backgroundColor: isSimple ? 'transparent' : (isHovered && !document.documentElement.classList.contains('light-theme') ? containerColor + '05' : containerColor + '01'),
        border: isSimple ? 'none' : `1.5px solid ${isHovered && !document.documentElement.classList.contains('light-theme') ? borderColor : (isHovered ? '#444' : borderColor)}`,
        borderRadius: '12px',
        padding: isSimple ? '10px' : '20px',
        position: 'relative',
        display: 'flex',
        flexDirection: 'column',
        boxShadow: selected && !isSimple ? `0 0 4px ${borderColor}02` : 'none',
        overflow: 'visible',
