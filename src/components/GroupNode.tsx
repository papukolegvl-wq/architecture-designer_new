...
import { Layers, Link as LinkIcon, Link2 } from 'lucide-react'
...
function GroupNode({
  ...
  const updateGroupSize = React.useCallback(() => {
    ...
    let insideNodes = allNodes.filter(node => {
      if (node.id === id || node.type === 'group' || node.type === 'system' || node.type === 'business-domain') return false
  ...
  return (
    <div
      style={{
        width: '100%',
        height: '100%',
        backgroundColor: isSimple ? 'transparent' : groupColor + '20',
