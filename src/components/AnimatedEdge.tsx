...
function AnimatedEdge({
  ...
  const edgeColor = React.useMemo(() => {
    ...
    switch (connectionType) {
      ...
      case 'inheritance':
        return '#4dabf7'
      case 'related':
        return '#adb5bd' // Grey color for related
      default:
        return '#4dabf7'
    }
  }, [data?.connectionType])
  ...
  let markerStartId = `arrowhead-start-${connectionType}${selected ? '-selected' : ''}`
  let markerEndId = `arrowhead-${connectionType}${selected ? '-selected' : ''}`
        ...
        <EdgeLabelRenderer>
          <div
            ...
            style={{
              ...
              borderWidth: '1px',
              borderStyle: 'solid',
              padding: '6px 12px', // Ещё больше отступы
              borderRadius: '6px',
              fontSize: '20px', // Увеличили с 16px до 20px
              fontWeight: 600,
            }}
            onMouseDown={(e) => {
