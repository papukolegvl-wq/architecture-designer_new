...
interface BatchProcessorConfigPanelProps {
    node: Node
    onUpdate: (nodeId: string, config: { vendor: BatchProcessorVendor }) => void
    onClose: () => void
}

const vendors: Array<{ value: BatchProcessorVendor; label: string; description: string }> = [
    { value: 'apache-airflow', label: 'Apache Airflow', description: 'Оркестрация сложных рабочих процессов' },
    { value: 'luigi', label: 'Luigi', description: 'Python-пакет для построения пайплайнов' },
    { value: 'prefect', label: 'Prefect', description: 'Современная система управления потоками данных' },
    { value: 'aws-batch', label: 'AWS Batch', description: 'Запуск пакетных заданий в облаке AWS' },
    { value: 'azure-batch', label: 'Azure Batch', description: 'Масштабируемые пакетные вычисления в Azure' },
    { value: 'google-cloud-composer', label: 'Google Cloud Composer', description: 'Airflow как управляемый сервис в GCP' },
]
