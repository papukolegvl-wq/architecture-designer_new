...
function CustomNode({ data, selected, id, onInfoClick, onLinkClick, onLinkConfigClick, onCommentClick, onStatusChange, onColorChange }: CustomNodeProps) {
  ...
  const updateContainerSize = React.useCallback(() => {
    ...
    let insideNodes = allNodes.filter(node => {
      if (node.id === id || node.type === 'container' || node.type === 'system' || node.type === 'group') return false
  ...
  const boxShadow = isNew
    ...
    : isRefinement
      ? selected
        ? `0 8px 40px ${color}70, 0 0 0 3px ${color}40, 0 0 30px #fab00580, 0 0 60px #fab00540`
        : `0 4px 20px rgba(0,0,0,0.5), 0 0 25px #fab00570, 0 0 50px #fab00550`
      : isExisting
        ? selected
  ...
  // Фиксированный тёмный фон для всех компонентов - не адаптируется с темой
  const backgroundColor = isNew ? '#1a2e1a' : isRefinement ? '#2e291a' : isExisting ? '#1a222e' : '#2d2d2d'
    ...
    >
      {!isSimple && (
        ...
        >
          ...
          {/* Status button - Always visible if enabled in props OR implies we want to change status */}
          <button
            onClick={(e) => {
              e.stopPropagation()
            ...
            {showColorPicker && (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <input
                    type="color"
                    value={data.customColor || componentColors[data.type] || '#4dabf7'}
                    onChange={(e) => {
      ...
      {data.type !== 'note' && (
        <div
          ...
          style={{
            width: '100%',
            height: '100%',
            backgroundColor: data.isExpanded ? '#1a1a1a' : (isSimple ? 'transparent' : '#1e1e1e'),
            ...
            boxShadow: selected && !isSimple ? `0 0 20px ${color}40` : (data.isExpanded ? '0 10px 30px rgba(0,0,0,0.3)' : 'none'),
            transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
            position: 'relative',
            overflow: 'hidden',
            minWidth: data.isExpanded ? '300px' : 'auto',
