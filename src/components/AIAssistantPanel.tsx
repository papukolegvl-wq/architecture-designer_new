    ...
    try {
      ...
      if (mode === 'learning' && currentCase) {
        contextPrompt = `
          ...
          ИНСТРУКЦИЯ (СТРОГО ПО ФОРМАТУ):
          Твоя цель — довести архитектуру до 100% соответствия КЕЙСУ и стандартам High Availability. 
          
          ОТВЕЧАЙ СТРОГО ПО БЛОКАМ.
          1. **АНАЛИЗ**: Проведи Gap Analysis: сравни холст с КАЖДЫМ бизнес-требованием и атрибутом качества. Если чего-то не хватает (даже если эксперт еще не проверял) — укажи на это.
          2. **ЧТО ДОБАВИТЬ**: Конкретный список. Формат: "Добавь компонент: 'Имя' (Type: тип)". Не забывай про Auth, Monitoring, Backups и SPOF.
          3. **ЧТО С ЧЕМ СВЯЗАТЬ**: Конкретный список. Формат: "Связь: '[Label A]' -> '[Label B]' (Type: тип)".
          4. **ПОЯСНЕНИЕ (ЗАЧЕМ)**: Техническое обоснование.
          
          ПРАВИЛА:
          - (ВАЖНО) MANDATORY AUDIT: Если проект не защищен (нет Auth), имеет SPOF (нет реплик/балансировщиков) или нет мониторинга — ты НЕ ИМЕЕШЬ ПРАВА говорить, что "все готово".
          - ZERO TOLERANCE: Никогда не предлагай связь, которая нарушает лучшие практики.
          - НЕ ГОВОРИ "ГОТОВО", если оценка меньше 100% или есть missedRequirements.
          - Будь краток и пиши сразу по делу.
