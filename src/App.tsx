  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      >
        ...
        {/* Индикатор выбранных компонентов */}
        {selectedNodeIds.length > 0 && (
          <div
            style={{
              ...
              right: '10px',
              zIndex: 1001,
              backgroundColor: selectedNodeIds.length >= 1 ? 'rgba(132, 94, 247, 0.9)' : 'rgba(0, 0, 0, 0.7)',
              color: '#fff',
              padding: '8px 12px',
              borderRadius: '4px',
              fontSize: '12px',
              fontWeight: selectedNodeIds.length >= 1 ? 'bold' : 'normal',
              border: selectedNodeIds.length >= 1 ? '2px solid #845ef7' : 'none',
            }}
          >
            Выбрано: {selectedNodeIds.length}
          </div>
        )}
        {(selectedNodeIds.length >= 1 || selectedGroupId) && (
          <div
            style={{
              position: 'fixed',
              top: '100px',
              ...
              alignItems: 'center',
              gap: '12px',
              boxShadow: '0 4px 12px rgba(132, 94, 247, 0.5)',
              pointerEvents: 'auto',
            }}
          >
            <span style={{ color: '#fff', fontSize: '14px', fontWeight: 'bold' }}>
              Выбрано компонентов: {selectedNodeIds.length}
              {selectedGroupId && ' (сгруппированы)'}
            </span>
            {selectedGroupId ? (
              <button
                onClick={(e) => {
              ...
              selectedNodeIds.length >= 1 && (
                <button
                  ...
                  title="Группирует выбранные компоненты вместе со всеми связанными компонентами"
                >
                  Группировать со связанными
                </button>
              )
            )}
          </div>
