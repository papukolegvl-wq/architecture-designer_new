...
export type DatabaseType = 'sql' | 'nosql' | 'browser' | 'embedded' | 'in-memory'
...
export type DatabaseInternalComponentType = 'disk' | 'cache' | 'index' | 'log' | 'network'

export interface DatabaseInternalComponent {
  id: string
  type: DatabaseInternalComponentType
  label: string
  description?: string
  technologies?: string[]
}

export interface DatabaseInternalArchitecture {
  components: DatabaseInternalComponent[]
...
export interface DatabaseConfig extends ResiliencePatterns {
  ...
  internalArchitecture?: DatabaseInternalArchitecture
}
